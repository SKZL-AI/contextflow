...
class HooksManager:
    ...
    def register(
        ...
        hook = RegisteredHook(
            callback=callback, priority=priority, name=name or "", tags=tags or set()
        )
        self._hooks[hook_type].append(hook)
        self._hooks[hook_type].sort(key=lambda h: h.priority)
        self._logger.debug(
            "Hook registered", hook_type=hook_type.value, name=hook.name, priority=priority
        )
        return hook
    ...
    def unregister(
        ...
        if callback is not None:
            self._hooks[hook_type] = [
                h for h in self._hooks[hook_type] if h.callback is not callback
            ]
    ...
    async def execute(
        ...
        for hook in hooks:
            try:
                ...
                result.errors.append(
                    {"hook_name": hook.name, "error_type": type(e).__name__, "message": str(e)}
                )
                result.hooks_skipped += 1
    ...
    def list_hooks(self) -> dict[str, list[dict[str, Any]]]:
        ...
        return {
            ht.value: [
                {"name": h.name, "priority": h.priority, "enabled": h.enabled, "tags": list(h.tags)}
                for h in hooks
            ]
...
def _make_decorator(hook_type: HookType):
    def decorator_factory(
        ...
        name: str | None = None,
        tags: set[str] | None = None,
        manager: HooksManager | None = None,
    ) -> Callable[[HookCallback], HookCallback]:
...
def pre_process(
    ...
    return (manager or get_global_hooks_manager()).register_decorator(
        HookType.PRE_PROCESS, priority, name, tags
    )
...
def post_process(
    ...
    return (manager or get_global_hooks_manager()).register_decorator(
        HookType.POST_PROCESS, priority, name, tags
    )
...
def pre_strategy(
    ...
    return (manager or get_global_hooks_manager()).register_decorator(
        HookType.PRE_STRATEGY, priority, name, tags
    )
...
def post_strategy(
    ...
    return (manager or get_global_hooks_manager()).register_decorator(
        HookType.POST_STRATEGY, priority, name, tags
    )
...
def on_error(
    ...
    return (manager or get_global_hooks_manager()).register_decorator(
        HookType.ON_ERROR, priority, name, tags
    )
...
def on_verification_fail(
    ...
    return (manager or get_global_hooks_manager()).register_decorator(
        HookType.ON_VERIFICATION_FAIL, priority, name, tags
    )


# Hook Utilities
def create_logging_hook(
    log_level: str = "info", include_context: bool = False
) -> AsyncHookCallback:
